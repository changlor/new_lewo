    ...
    public function return_url(){
        ...
        if($verify_result) {
            ...
            if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
              ...
              $this->error("支付失败!trade_status=".$_GET['trade_status'],U("Home/Tenant/index"));
            }
        }
        else {
            //验证失败
            //如要调试，请看alipay_notify.php页面的verifyReturn函数
            $this->error("验证失败!",U("Home/Tenant/index"));
        }
    }
    ...
    public function notify_url(){
        ...
        if($verify_result) {//验证成功
            ...
            if($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
                ...
                /*$period         = $contract_info['period'];//缴费周期
                $year           = date("Y",strtotime($start_time));
                $month          = date("m",strtotime($start_time));
                $ht_end_day     = date("d",strtotime($end_time));
                $rent_date      = correct_date(date("Y",strtotime($year."-".$month."+".$period." month")),date("m",strtotime($year."-".$month."+".$period." month")),$ht_end_day); //房租到期日*/ 
                ...
                if ( $price == $total_fee) {
                    ...
                    /*$contract_save['rent_date'] = $rent_date;*/
